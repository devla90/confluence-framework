...
| `type:` | Document type | One per page, matching the template it came from: `type:func-spec`, `type:architecture`, `type:adr`, `type:api-spec`, `type:env-config`, `type:runbook`, `type:guide`, `type:reference`, `type:security-doc`, `type:migration`, `type:release-note`, `type:deployment-request`, `type:test-plan`, `type:test-strategy`, `type:infra-request`, `type:role-request`. Plus `type:policy` as a **second** label on any page that states standing rules — a security policy is `type:security-doc` + `type:policy`, a governance one is `type:guide` + `type:policy` |
...
| `reference` | A list consulted rather than read — glossaries, catalogs, inventories, matrices, registries | `templates/reference.md` | `type:reference`, `status:draft`, `team:{team}` |
